...
// type dict('a) = D.t('a);
// type error =
//   | RowDecodeError(int, Yojson.t, string)
//   | EmptyRows;
...
//   // let decodeJson: (t(Yojson.t), decoder('a)) => 'a =
//   //   (row, decode) =>
//   //     try(mapGet(row, decode)) {
//   //     | DecodeError(err) =>
//   //       raise(Error(RowDecodeError(row.index, row.contents, err)))
//   //     };
// };
// // Something which can translate rows of Json objects.
// type rowsDecoder('t) = array(Row.t(Yojson.t)) => 't;
...
// let toRows: array('a) => array(Row.t('a)) =
//   rs => A.mapWithIndex(rs, Row.make);
...
// let decodeEach: (decoder('a), array(Row.t(Yojson.t))) => array('a) =
...
// let columns2:
//   (string, decoder('a), string, decoder('b)) => decoder(('a, 'b)) =
//   (columnA, decodeA, columnB, decodeB, j) => (
//     j |> field(columnA, decodeA),
//     j |> field(columnB, decodeB),
//   );
// let columns3:
//   (string, decoder('a), string, decoder('b), string, decoder('c)) =>
//   decoder(('a, 'b, 'c)) =
...
// let withId =
//     (~idField: string, ~idDecode: decoder('id), decode: decoder('t))
//     : decoder(('id, 't)) =>
...
//     jsonRows
//     |> decodeEach(
//          tup2(field(keyField, keyDecode), field(valueField, valueDecode)),
//        )
//     |> D.fromArray;
//   };
// let dict2d =
...
//               result,
//               k1,
//               D.singleton(k2, D.singleton(k3, D.singleton(k4, value))),
//             )
//           | Some(values1) =>
//             switch (D.get(values1, k2)) {
//             | None =>
//               D.set(values1, k2, D.singleton(k3, D.singleton(k4, value)))
//             | Some(values2) =>
