...
module String = {
  let contains = (~substring, str) =>
    Tablecloth.String.contains(~substring, str);

  let replace = (~old, ~new_, str) =>
    Str.global_replace(Str.regexp_string(old), new_, str);

  let isMatch: (string, string) => bool =
    (s, r) => Str.string_match(Str.regexp_string(r), s, 0);
  ...
  let joinWith: (array(string), string) => string =
    (arr, sep) =>
      switch (arr |> Array.to_list) {
      | [] => ""
      | [head] => head
      | [head, ...tail] =>
        tail |> List.fold_left((acc, item) => {acc ++ sep ++ item}, head)
      };

  let joinSpaces: array(string) => string = arr => joinWith(arr, " ");
  let mapJoin:
    (array('a), ~prefix: string=?, ~suffix: string=?, string, 'a => string) =>
    string =
    (arr, ~prefix="", ~suffix="", sep, f) =>
      prefix ++ joinWith(A.map(arr, f), sep) ++ suffix;
  ...
  let mapJoinSpaces = (arr, ~prefix=?, ~suffix=?, f) =>
    mapJoin(arr, ~prefix?, ~suffix?, " ", f);
  let mapJoinCommasParens = (arr, f) =>
    mapJoin(arr, ~prefix="(", ~suffix=")", ", ", f);
...
//   let transform: ('a => 'b, t('a)) => t('b) =
//     (f, prom) => prom |> then_(x => resolve(f(x)));
...
//   let rLog2: ('a, 'b) => Js.Promise.t(unit) =
//     (a, b) => Js.Promise.resolve(Js.log2(a, b));
...
module Array = {
  ...
  let contains: (array('a), 'a) => bool =
    (arr, elem) => some(arr, e => e == elem);
  ...
  let mapJoin:
    ...
    string =
    (arr, ~prefix="", ~suffix="", sep, f) =>
      prefix ++ joinWith(map(arr, f), sep) ++ suffix;
  let mapJoinWith: (array('a), string, 'a => string) => string =
  ...
  let findExn: (array('a), 'a => bool) => 'a =
    (arr, test) =>
      ...
      | None => failwith("No matching element in array")
      | Some(m) => m
      };
  ...
  // let cross =
  //     (arr1: array('a), arr2: array('b), f: ('a, 'b) => 'c): array('c) => {
  ...
  let keepSome = (arr: array(option('a))): array('a) =>
    keepMap(arr, x => x);
...
//   module Decode = {
//     // include Json.Decode;
//     // external json: Yojson.t => Yojson.t = "%identity";

//     // let strMap: decoder('a) => decoder(SMap.t('a)) =
//     //   (inner, obj) =>
//     //     obj |> dict(inner) |> Js.Dict.entries |> SMap.fromArray;
...
//     let floatString: decoder(float) =
//       oneOf([float, obj => obj |> string |> float_of_string]);

//     // Can parse either a JSON int, or a int-like string.
//     let intString: decoder(int) =
//       oneOf([int, obj => obj |> string |> int_of_string]);
...
//   let pretty: Yojson.t => string = [%bs.raw
...
//       (
//         ~pretty=false,
//         encA: Encode.encoder('a),
//         encB: Encode.encoder('b),
//         a: 'a,
//         b: 'b,
//       ) => {
